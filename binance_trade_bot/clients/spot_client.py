# Spot market client implementation
import math
import time
import traceback
...
class SpotClient(BinanceBaseClient):
    ...
    def get_trade_fees(self) -> Dict[str, float]:
        ...
        return {symbol["symbol"]: 0.001 for symbol in exchange_info["symbols"]}
    ...
    def get_fee(self, origin_coin: Coin, target_coin: Coin, selling: bool):
        ...
        if not self.testnet and not self.get_using_bnb_for_fees():
            return base_fee
        ...
        with self.cache.open_balances() as cache_balances:
            ...
            if force or balance is None:
                ...
                cache_balances.update({
                    cb["asset"]: float(cb["free"])
                    for cb in self.binance_client.get_account()["balances"]
                })
    ...
    def _buy_quantity(self, origin_symbol: str, target_symbol: str,
    ...
    def _wait_for_order(self, order_id, origin_symbol: str, target_symbol: str) -> Optional[BinanceOrder]:
        while True:
    ...
    def wait_for_order(self, order_id, origin_symbol: str, target_symbol: str,
                       order_guard: OrderGuard) -> Optional[BinanceOrder]:
        with order_guard:
    ...
    def acquire_order_guard(self):
        return self.stream_manager.acquire_order_guard()
